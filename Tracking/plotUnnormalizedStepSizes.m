function unscaledDistsOut = plotUnnormalizedStepSizes(FeatMats,IncProp,StatsUse,trackability,unscaledDists,axHandBig,axHandSmall)
...
%       -unscaledDists: Either empty (in which case unnormalised step sizes
%       are calculated de novo) or a vector containing the unnormalised
%       step sizes for the currently selected feature combination.
...
%
%   OUTPUTS:
%       -unscaledDistsOut: The unnormalised step sizes from the training
%       dataset. Outputting allows inclusion proportion selection to be 
%       sped up (as this set of statistics doesn't need to be recalculated 
%       every time).
%
%   Author: Oliver J. Meacock (c) 2019

if isempty(unscaledDists)
    %Find the total number of objects in dataset
    totFeatNo = 0;
    for i = 1:length(FeatMats.lin)
        totFeatNo = totFeatNo + size(FeatMats.lin{i},1);
    end
    
    %Find the full range of values that each of the linear variables can take
    allFeats = zeros(totFeatNo,size(FeatMats.lin{1},2)-1);
    loadInd = 1;
    for i = 1:length(FeatMats.lin)
        allFeats(loadInd:loadInd + size(FeatMats.lin{i},1) - 1,:) = FeatMats.lin{i}(:,2:end);
        loadInd = loadInd + size(FeatMats.lin{i},1);
    end
    minFeats = min(allFeats,[],1);
    maxFeats = max(allFeats,[],1);
    
    unscaledDists = nan(size(allFeats,1),1);
    ...
    for i = 1:length(FeatMats.lin) - 1
        if ~isempty(FeatMats.lin{i}) && ~isempty(FeatMats.lin{i+1})
            ...
            unscaledDists(loadInd:loadInd + size(distsTmp,1) - 1) = distsTmp;
            loadInd = loadInd + size(distsTmp,1);
        end
    end
    
    unscaledDists(isnan(unscaledDists)) = [];
end

%Put unscaled step sizes into output variable
unscaledDistsOut = unscaledDists;
